...
/*for (let i = 0; i < runners.length; i++) {
    fullNames[i] = runners[i].first_name + ' ' + runners[i].last_name;
}*/
runners.forEach(cb => {
    fullNames.push(`${cb.first_name} ${cb.last_name}`);
});
console.log(fullNames,'forEach Method');
...
/*for (let i = 0; i < runners.length; i++) {
    firstNamesAllCaps[i] = runners[i].first_name.toUpperCase();
}*/
console.log(firstNamesAllCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners array, containing only those runners with large sized shirts so they can choose a different size. This will be an array of objects.
let runnersLargeSizeShirt = runners.filter((value) => {
...
let ticketPriceTotal = runners.reduce((totalVal, value) => {
    return totalVal += value.donation;
}, 10);
/*for (let i = 0; i < runners.length; i++) {
...
let worksForUs = runners.filter((value) => {
    return value.company_name == 'DabZ';
});
console.log(worksForUs);

// Problem 2

let profitMargin = runners.reduce((totalVal, value) => {
    return totalVal += value.donation;
}, 10);
/*for (let i = 0; i < runners.length; i++) {
    profitMargin += runners[i].donation;
}*/
console.log('Profit $' + (profitMargin - 500));
// Problem 3

let tooSmall = runners.filter((value) => {
